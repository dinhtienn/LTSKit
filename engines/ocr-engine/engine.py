...
NG_DOI = 0.45     # lech Jaccard giua 2 mask chu > nguong nay -> CHU DOI (mo doan moi)
...
def chuan_hoa_text(text):
    """Chuan hoa de SO SANH; van giu nguyen text goc khi ghi SRT."""
    text = text.lower()
    text = "".join(ch if not unicodedata.category(ch).startswith("P") else " " for ch in text)
    return " ".join(text.split())

def do_giong(a, b):
    a_norm, b_norm = chuan_hoa_text(a), chuan_hoa_text(b)
    if not a_norm or not b_norm:
        return 0.0
    return SequenceMatcher(None, a_norm, b_norm, autojunk=False).ratio()

def gop_cau_giong(subs):
    """Gop cue lien timeline va giong >= 90%; giu text goc dai nhat."""
    if not subs:
        return []
    ra = [subs[0]]
    for bat_dau, ket_thuc, text in subs[1:]:
        cu_bat_dau, cu_ket_thuc, cu_text = ra[-1]
        do_troi = bat_dau - cu_ket_thuc
        lien_nhau = 0 <= do_troi < 1e-6
        if lien_nhau and do_giong(cu_text, text) >= 0.90:
            text_tot = text if len(text) > len(cu_text) else cu_text
            ra[-1] = (cu_bat_dau, max(cu_ket_thuc, ket_thuc), text_tot)
        else:
            ra.append((bat_dau, ket_thuc, text))
    return ra

def hoan_tat(subs, output, band_top, band_bot):
    subs = gop_cau_giong(subs)
...
def run(args):
    ...
    return hoan_tat(subs, args.output, band_top, band_bot)

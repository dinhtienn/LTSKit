...
import { ASSET_BASE, binDir, DATA_DIR, downloadFile, ensureDataDirs } from './deps'
...
function libraryDbPath(): string {
  return join(DATA_DIR, 'dy-library.db')
}
function configPath(): string {
  return join(DATA_DIR, 'dy-config.yml')
}
function channelsPath(): string {
  return join(DATA_DIR, 'dy-channels.json')
}
...
export async function downloadDouyin(
  ...
  const cookies = req.useCookies ? await readDyCookies() : {}

 ...
 *
 * Runtime asset releases are hosted as prereleases in dinhtienn/LTSKit.
 * Keep this fixed tag; do not use `releases/latest/download` because app releases
 * must not require re-uploading all engine assets.
 *
 * Before distributing a build to a fresh machine, verify ltskit-assets-v1 has
 * every required filename declared by the runtime asset manifest.
 */
export const ASSET_TAG = 'ltskit-assets-v1'
export const ASSET_BASE = `https://github.com/dinhtienn/LTSKit/releases/download/${ASSET_TAG}`

/**
 * Thu muc du lieu app (engine, cookie, log, model…) — LUON ten `appdata`.
 *
 * - Dev (npm run dev): <thu-muc-project>/appdata
  *   (cung cho voi code; vd D:\Tools\LTSKit\appdata — KHONG %APPDATA% o C:)
 * - Ban dong goi: <thu-muc-chua-exe>/appdata (portable, canh setup/app)
 *
 * Khong hardcode o dia may dev. Tao thu muc khi can (download/setup).
 */
function resolveDataDir(): string {
  if (app?.isPackaged) {
    // Canh file .exe / .app — user cai o dau thi appdata o do
    return join(dirname(app.getPath('exe')), 'appdata')
  }
  // Dev: goc project (electron-vite: app.getAppPath() = thu muc project)
  // Fallback process.cwd() neu getAppPath loi
  try {
    return join(app?.getAppPath() ?? process.cwd(), 'appdata')
  } catch {
    return join(process.cwd(), 'appdata')
  }
}
...
export function binDir(): string {
  return join(DATA_DIR, 'bin')
}

/** Dam bao appdata/ + appdata/bin/ ton tai (goi truoc khi tai tai nguyen). */
...
/** Chay setup: tao appdata/ neu chua co, chi tai cai nao dang thieu. */
